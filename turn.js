...
<body class="min-h-screen flex flex-col items-center">
    ...
    <script type="module">
        ...
        import { getDatabase, ref, onValue, set, remove, runTransaction } from "https://www.gstatic.com/firebasejs/10.12.2/firebase-database.js";
        ...
        window.firebaseDB = { database, ref, onValue, set, remove, runTransaction };
    </script>
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            async function createCardElement(cardData) {
                ...
                cardDiv.innerHTML = `
                    ...
                    return `
                                        ...
                                        <div class="pericias-scroll-area flex flex-col gap-2 mt-4" style="max-height: 100px; overflow-y: auto;">
            ...
            statusForm.addEventListener('submit', async (e) => {
                ...
                if (cardOnScreen) {
                    ...
                    if (newMoney !== oldMoney) {
                        ...
                        if (newMoney > oldMoney) {
                            createManaMist(moneyIcon, 'in', '#fcd34d');
                        }
                        if (newMoney < oldMoney) {
                            createManaMist(moneyIcon, 'out', '#fcd34d');
                        }
            ...
            async function useItem(templateId, characterId) {
                ...
                const card3d = cardOnScreen.querySelector('.card-3d');
                resetcard3d(card3d);
            ...
            jsonImportInput.addEventListener('change', (e) => {
                ...
                reader.onload = async (event) => {
                    try {
                        ...
                        // Verifica se é um backup completo ou um card individual
                        if (currentStoreForImport === 'full_database') {
                            // Limpa o banco de dados atual para evitar duplicatas
                            const storeNames = [CARD_STORE_NAME, SPELL_STORE_NAME, ITEM_STORE_NAME];
                            for (const storeName of storeNames) {
                                const allItems = await getData(storeName);
                                for (const item of allItems) {
                                    await removeData(storeName, item.id);
                                }
                            }

                            // Importa os dados para cada store
                            for (const storeName of storeNames) {
                            ...
                            showCustomAlert("Backup importado com sucesso!");

                        } else { // Lógica para importação de card individual (já existente)
            ...
            function initializeNebulaEffect(container) {
                ...
                const updateLayers = (x, y) => {
                    ...
                    txtItem.style.transform = `translateZ(-20px) translate(${x * factorMedium}px, ${y * factorMedium}px)`;
